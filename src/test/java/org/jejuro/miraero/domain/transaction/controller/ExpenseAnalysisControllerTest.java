...
class ExpenseAnalysisControllerTest {
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new ExpenseAnalysisController(service)).setControllerAdvice(new GlobalExceptionHandler()).build();
    }

    @Test
    void getDashboard_successAndEmptyData() throws Exception {
        given(service.getDashboard(1L, 2026, 7)).willReturn(new ExpenseDashboardResponse(2026, 7, Collections.emptyList(), new CategoryExpenseSummaryResponse(0L, Collections.emptyList())));
        ...
        verify(service).getDashboard(1L, 2026, 7);
    }

    @Test
    void getDashboard_invalidMonthReturnsGlobalError() throws Exception {
        given(service.getDashboard(eq(1L), eq(2026), eq(13))).willThrow(new BusinessException(CommonErrorCode.INVALID_INPUT_VALUE));
    ...
    void getDashboard_requiresYearAndMonth() throws Exception {
        given(service.getDashboard(1L, 2026, null)).willThrow(new BusinessException(CommonErrorCode.INVALID_INPUT_VALUE));
